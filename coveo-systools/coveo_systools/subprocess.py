...
class DetailedCalledProcessError(subprocess.CalledProcessError):
    ...
    @staticmethod
    def _decode(value: Union[bytes, str, None], *,
                decode_errors: Optional[Literal['ignore', 'replace']] = 'ignore') -> Optional[str]:
...
class _CallProtocol(Protocol):
    ...
    def __call__(self, command: List[str], shell: bool = False, cwd: str = None, **kwargs: Any) \
