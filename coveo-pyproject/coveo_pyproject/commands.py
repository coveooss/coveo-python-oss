...
@click.option('--verbose', is_flag=True, default=False)
def build(project_name: str, directory: Union[str, Path] = None, python: Union[str, Path] = None,
    ...
    python_environments = [PythonEnvironment(python)] if python \
...
@click.option('--verbose', is_flag=True, default=False)
def mypy(project_name: str = None, exact_match: bool = False, verbose: bool = False,
         extra_mypy_option: List[str] = None) -> None:
...
def locate(project_name: str, verbose: bool = False) -> None:
    ...
    try:
        ...
        partial_matches = (project.package.name for project
                           in PythonProject.find_pyprojects(query=project_name, verbose=verbose))
        try:
            raise ExitWithFailure(
