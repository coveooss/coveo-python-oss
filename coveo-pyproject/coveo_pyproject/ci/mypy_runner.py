...
class MypyRunner(ContinuousIntegrationRunner):
    ...
    def _mypy_config_path(self) -> Path:
        ...
        try:
            return next(find_paths(Path('mypy.ini'), self._pyproject.project_path,
                                   in_root=True, in_parents=True, in_children=True))
    ...
    def _find_typed_folders(self) -> Generator[Path, None, None]:
        ...
        yield from filter(lambda path: (path / PythonFile.TypedPackage).exists(),
    ...
    def _launch(self, environment: PythonEnvironment, *extra_args: str) -> RunnerStatus:
        ...
        command = mypy_environment.build_command(
            ...
            '--config-file', self._mypy_config_path(),
            '--cache-dir', self._pyproject.project_path / '.mypy_cache',
            '--show-error-codes',
