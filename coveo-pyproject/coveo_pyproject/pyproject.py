...
class PythonProject(PythonProjectAPI):
    ...
    def __init__(self, project_path: Path, *, verbose: bool = False) -> None:
        ...
        self.package: PoetryAPI = flexfactory(
            PoetryAPI,
            **dict_lookup(toml_content, 'tool', 'poetry'),
            _pyproject=self)
    ...
    def find_pyprojects(cls: Type[T_PythonProject], path: Path = None,
        ...
        if count_projects == 0:
            raise PythonProjectNotFound(f"Cannot find any project that could match {query}" if query
    ...
    def virtual_environments(self, *, create_default_if_missing: bool = False) -> List[PythonEnvironment]:
        ...
        """
        environments = []
        for str_path in self.poetry_run('env', 'list', '--full-path',
                                        capture_output=True,
                                        breakout_of_venv=True).split('\n'):
            if str_path.strip():
    ...
    def current_environment_belongs_to_project(self) -> bool:
        ...
        return any(environment.python_executable == current_executable
            ...
            for environment in self.virtual_environments(create_default_if_missing=True):
                try:
                    ...
                    if status is not RunnerStatus.Success:
                        echo.warning(f"{self.package.name}: {runner} reported issues:",
                                     pad_before=False, pad_after=False)
                        runner.echo_last_failures()

                except DetailedCalledProcessError as exception:
                    echo.error(f"The ci runner {runner} failed to complete "
    ...
    def poetry_run(self, *commands: Any, capture_output: bool = False, breakout_of_venv: bool = True) \
            -> Optional[str]:
