...
class PyDev(PythonProject):
    ...
    def pull_dev_requirements(self, *, dry_run: bool = False) -> bool:
        ...
        all_dev_dependencies.comment("pydev projects' dev-dependencies are autogenerated; do not edit manually!")
        toml['tool']['poetry']['dev-dependencies'] = all_dev_dependencies

        if safe_text_write(self.toml_path, '\n'.join(self._format_toml(tomlkit.dumps(toml))),
                           only_if_changed=True, dry_run=dry_run):
    ...
    def find_pyproject_paths(cls, path: Path) -> Generator[Path, None, None]:
        ...
        yield from filter(lambda _path: flexfactory(CoveoPackage, **load_toml_from_path(_path)).pydev,
